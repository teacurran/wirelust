...
resource "aws_cloudfront_distribution" "beta" {
  origin {
    domain_name = aws_s3_bucket_website_configuration.beta_bucket.website_endpoint
    origin_id   = "S3-beta.${var.bucket_name}"
  ...
  default_cache_behavior {
    allowed_methods  = ["GET", "HEAD"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "S3-beta.${var.bucket_name}"
